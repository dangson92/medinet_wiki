...
- **Plan 03-01 FE-04 — UserRole type alias + UserAPI.role extend + mockData hub_admin fixture:** `frontend/src/services/api.ts` export `type UserRole = 'admin' | 'hub_admin' | 'editor' | 'viewer'` mirror BE Pydantic Literal (Plan 02-03) + Phase 1 migration 0006 CHECK constraint (D-V3.1-Phase3-D LOCKED single source-of-truth — drift FE/BE/migration → 422 reject runtime). `UserAPI` interface ADD `role: UserRole` (D-V3.1-Phase3-B LOCKED — BE Phase 2 đã trả qua /api/auth/me). `types.ts` `User.role` extend từ 2 → 4 value (additive widening — TypeScript accept superset). `mockData.ts` ADD `MOCK_HUBS` entry `tdt` (Thuốc Dân Tộc — audit 2026-05-24 phát hiện thiếu; dmd + tdt = 2 hub thật DB per memory `project_real_hubs_deployment`) + APPEND 2 hub_admin user fixture `u-hadmin-dmd` + `u-hadmin-tdt` với `hubId` lookup runtime `MOCK_HUBS.find(h => h.code === 'dmd'|'tdt')?.id ?? ''` (D-V3.1-Phase3-C LOCKED). AuthContext.tsx VERIFY ONLY (KHÔNG code change). Reference: `.planning/phases/03-frontend-form-refactor/03-01-PLAN.md` + `03-01-SUMMARY.md`.

- **Plan 03-02 FE-01 — UserManagement Add User form 3 option + warning banner + error envelope switch:** import `UserRole` type. State `newUserRole` + `manageRole` extend `UserRole`. Add User form refactor radio group 2 → 3 option ("Admin toàn hệ thống" + "Quản lý hub này" + "Viewer") qua native HTML `<fieldset role="radiogroup">` + `<legend id="role-group-label">` + 3 `<label>` wrap radio + description tiếng Việt exact UI-SPEC §7.1. ARIA full `aria-describedby="role-{X}-desc"` link description id; warning banner conditional `{newUserRole === 'admin' && <div role="alert" aria-live="polite" class="bg-yellow-50 border-yellow-200 text-yellow-800 dark:bg-yellow-900/20">⚠ Quyền cao nhất — quản lý toàn bộ hệ thống</div>}`. `handleCreateUser` error.code switch ADD 4 BE envelope mới Phase 2 (HUB_ADMIN_REQUIRED "Bạn không có quyền tạo user với role Admin toàn hệ thống. Liên hệ Super Admin." + HUB_ID_REQUIRED "Vui lòng chọn hub trước khi tạo user." + AUTH_STATE_INCONSISTENT "Lỗi hệ thống xác thực — liên hệ admin. Mã: AUTH_STATE_INCONSISTENT" + FORBIDDEN "Bạn không có quyền thực hiện thao tác này.") với toast tiếng Việt exact UI-SPEC §7.4. Generic fallback `error?.message ?? 'Tạo user thất bại.'` preserve. Reference: `03-02-PLAN.md` + `03-02-SUMMARY.md`.

- **Plan 03-03 FE-02 + FE-03 — Hub switcher Layout.tsx + Manage modal disabled UserManagement.tsx:** `Layout.tsx` ADD `HubSwitcher` component inline module-level (~80 LOC keep inline borderline OK <80 LOC threshold per Claude discretion) render ABOVE sidebar nav qua `{(!collapsed || isMobileMenuOpen) && <HubSwitcher />}` (visible khi sidebar expanded). Filter logic D-V3.1-Phase3-A LOCKED hardcode 'central' slug: super admin show ALL; non-super-admin `h.code !== 'central' && userHubIds.includes(h.id)`. `userHubIds` derive Option A LOCKED `user?.roles?.map(r => r.hub_id) ?? []` (less invasive — KHÔNG extend UserAPI.hub_ids). 3 render branch (loading skeleton `aria-busy="true"` + empty state `role="status"` "Bạn chưa được gán hub nào — liên hệ admin." + native `<select aria-label="Chọn hub đang xem">`); onChange → `window.location.href = /${code}/` full reload (Phase 5 PROXY pattern carry forward). Active hub compare `value={CURRENT_HUB}`. `UserManagement.tsx` Manage modal block REPLACE 2 → 3 button-style option (preserve existing CheckCircle2 indicator + ring-1 ring-brand-indigo selected) với "Admin toàn hệ thống" `disabled={!isCurrentSuper}` + `aria-disabled` + native `title="Cần Admin toàn hệ thống"` (chỉ khi disabled) + `aria-describedby="manage-admin-tooltip-desc"` link sr-only helper "Tùy chọn này yêu cầu quyền Admin toàn hệ thống." + Tailwind `opacity-50 cursor-not-allowed` (defense in depth UX layer — BE Plan 02-03 T-02-02-E business logic block role escalation authoritative). `handleSubmitManageHub` error switch handle 3 envelope Manage context (HUB_ADMIN_REQUIRED "Bạn không có quyền gán role này. Liên hệ Super Admin." + CROSS_HUB_USER_DELETE_DENIED "Không thể xóa user thuộc nhiều hub. Liên hệ Super Admin để xử lý." + FORBIDDEN) — toast DIFFERENT từ Plan 03-02 form context (Manage modal nói "gán role" thay "tạo user"). Regression fix Layout.spec.tsx mock `api.getHubs` spread `importActual` tránh unhandled fetch jsdom KHÔNG resolve relative URL `/yte/api/hubs`. Reference: `03-03-PLAN.md` + `03-03-SUMMARY.md`.

- **Plan 03-04 closeout — 3 vitest test file mới + 4 docs source-of-truth atomic update:** 3 vitest test file MỚI ship `frontend/src/pages/__tests__/UserManagement.form-3-option.spec.tsx` (FE-01 — 3 test: render 3 radio + warning banner toggle + ARIA fieldset radiogroup) + `frontend/src/__tests__/Layout.hub-switcher.spec.tsx` (FE-02 — 3 test: super admin show ALL + hub_admin dmd CHỈ dmd + viewer empty state) + `frontend/src/pages/__tests__/UserManagement.manage-modal-disabled.spec.tsx` (FE-03 — 2 test smoke: render KHÔNG crash per currentRole; Manage modal mở qua action menu defer Phase 4 MIGRATE-02 full E2E). RTL render + `vi.doMock` pattern carry forward Phase 5 Plan 05-02 vitest infrastructure (`vi.resetModules` + AuthContext mock + ThemeContext mock + GeminiAssistant noop + api mock spread `importActual` cho `CURRENT_HUB` const). 5 baseline + 3 mới = **8 test file / 45 test PASS clean**. 4 docs source-of-truth update atomic (STATE.md + REQUIREMENTS.md + ROADMAP.md + CLAUDE.md §6 — file này). Smoke checkpoint runtime SKIP pre-resolved (carry forward v3.0 Plan 04-07 + 05-06 + 06-05 pattern + Plan 01-03 + 02-05 v3.1 — auto-fallback `--chain` mode active).

**Architecture insights (Phase 3 v3.1):**

1. **UserRole single source-of-truth centralize ở FE `services/api.ts`** — Drift FE alias / BE Pydantic Literal / migration 0006 CHECK constraint → POST `role='hub_admin'` runtime fail 422 reject HOẶC FE KHÔNG render role mới. Consumer import qua `import type { UserRole } from '../services/api'`.
2. **HubSwitcher inline ~80 LOC keep inline (<80 LOC threshold Claude discretion borderline OK)** — Extract `frontend/src/components/HubSwitcher.tsx` riêng defer khi future Phase v3.2+ scope phình lên (multi-select / search / icon hub).
3. **Option A userHubIds derive từ `roles: RoleAPI[]` (less invasive)** — KHÔNG extend UserAPI.hub_ids vì RoleAPI[] existing M2 đã có sẵn payload từ /api/auth/me. UserAPI.hub_ids extend chỉ khi BE schema thay đổi (defer v4.0 per-resource ACL).
4. **Defense in depth UX layer (FE filter + disabled)** — BE Plan 02-02 GET /api/hubs filter authoritative + Plan 02-03 T-02-02-E business logic block role escalation 403 HUB_ADMIN_REQUIRED. FE layer KHÔNG bypass khi DevTools tampering — chỉ UX layer (D-V3.1-Phase3-A LOCKED hardcode 'central' slug + Manage modal disabled state).
5. **Error envelope code switch FE consume 5 BE envelope mới Phase 2** (HUB_ADMIN_REQUIRED + HUB_ID_REQUIRED + CROSS_HUB_USER_DELETE_DENIED + AUTH_STATE_INCONSISTENT + FORBIDDEN) — toast tiếng Việt exact UI-SPEC §7.4 cho hub_admin user khi BE reject; KHÁC text giữa form context (Plan 03-02 "tạo user") và Manage modal context (Plan 03-03 "gán role").
6. **Vitest pattern Phase 5 Plan 05-02 carry forward** — jsdom env + `vi.doMock` context pattern + RTL render + jest-dom matchers + `vi.importActual` spread cho service module (giữ CURRENT_HUB const, override chỉ `api.getHubs` etc). Phase 3 Plan 03-04 add 3 test file → 8 file total / 45 test PASS clean.
7. **Layout.spec.tsx regression fix pattern** — Khi Layout component touch (vd Plan 03-03 HubSwitcher add `api.getHubs()` fetch call) → existing test mock spread `importActual` cho service module để tránh unhandled fetch jsdom. Áp dụng cho future Phase n+ khi Layout/sidebar touch BE call mới.

**Backward compat (Phase 3 v3.1 KHÔNG break v3.0 + Phase 1+2):**
...
- M2 envelope shape `{success, data, error, meta}` LOCKED — chỉ extend error code consume (5 envelope mới Phase 2).
...
- `api.createUser` + `api.changeUserRole` signature preserve (BE Phase 2 đã accept role='hub_admin' Pydantic Literal extend).
- Sidebar logo + branding header line 166-215 Phase 5 PROXY-04 UNCHANGED.
- `useAuth()` hook contract LOCKED.

**Phase 3 backward incompat resolved (operator broadcast W5 iter 1 Plan 02-05 closed):**
- Phase 2 hub_admin GET /api/users yêu cầu hub_id query (HUB_ID_REQUIRED guard) — Plan 03-02 FE error switch handle envelope + toast tiếng Việt UI-SPEC §7.4 → hub_admin user thấy "Vui lòng chọn hub trước khi tạo user." (UI hub picker đã có).
- Phase 2 W5 iter 1 1-2 ngày downtime user management page → resolve sau Plan 03-02 + 03-03 ship.
...
- Error envelope handle 4-5 BE code mới Phase 2 (Plan 03-02 + 03-03) — toast tiếng Việt exact UI-SPEC §7.4.
...
- **Plan 04-01 MIGRATE-01 — Makefile shortcut + migration verify DEFERRED (DONE_WITH_DEVIATION):** `api/Makefile` APPEND 2 target mới (`test-integration` chạy `pytest tests/integration -v -m integration --tb=short` + `test-migration` chạy 2 file migration test specific) + .PHONY update. Plan 01-01 LOCKED migration `0006_role_hub_admin.py` source code KHÔNG modify. Task 2 (chạy 7 test PASS) DEFERRED do pre-existing test infrastructure debt unrelated to MIGRATE-01 scope: (1) 2 test `test_migration_upgrade_downgrade.py` có stale Phase 2 v2.0 assertions — expect 10 baseline tables nhưng 0005 ship `sync_outbox` chưa update + expect alembic_version persists after downgrade base nhưng 0001 implementation drops it; (2) 5 test `test_migration_0006_idempotent.py` thiếu test isolation — mỗi test downgrade base destroys state cho test kế. Migration 0006 verified LIVE ở Plan 01-01 ship 2026-05-23 (Phase 1 SHIPPED + medinet-postgres deployed). Plan 04-02 hub_app_factory testcontainer exercise migration head provides indirect MIGRATE-01 verification end-to-end. Test infra cleanup defer v3.2/v4.0 follow-up. D-V3.1-Phase4-A LOCKED `sa.inspect()` introspect pattern + D-V3.1-Phase4-B LOCKED downgrade() full implement với defensive RuntimeError E-V3.1-1 STOP — carry forward Plan 01-01 ship. Reference: `.planning/phases/04-migration-smoke-e2e/04-01-PLAN.md` + `04-01-SUMMARY.md`.

- **Plan 04-02 MIGRATE-02 — Smoke E2E 4 scenario test mới + audit forensic chain:** File mới `api/tests/integration/test_smoke_e2e_v3_1_rbac.py` (~430 LOC) ship 4 scenario async test PASS clean **in 19.86s** + `seed_hubs_dmd_tdt` fixture (seed 2 hub thật `dmd` Đỗ Minh Đường + `tdt` Thuốc Dân Tộc per memory `project_real_hubs_deployment`) + 2 helper (`_seed_hub_admin_user` pattern carry forward test_dep_hubs_scope.py:37-76 + `_assert_audit_actor_metadata` poll-based timeout 3s carry forward conftest.py + test_audit_actor_metadata.py). Reuse `auth_client` + `admin_user` + `admin_token` + `viewer_user` + `viewer_token` + `_login_get_token` + `GO_SEED_HASH` + `app_with_auth` từ conftest.py existing (KHÔNG redeclare). 4 scenario: (1) super admin GET /api/hubs → cả dmd + tdt + POST user dmd → 201 + audit `actor_role='admin' + actor_hub_id=NULL`; (2) hub_admin dmd GET → CHỈ dmd (filter central + tdt qua Plan 02-02 D-V3.1-Phase2-A LOCKED backend authoritative) + POST user dmd → 201 + POST user tdt → 403 HUB_ADMIN_REQUIRED envelope + audit `actor_role='hub_admin' + actor_hub_id=<dmd-uuid>`; (3) hub_admin tdt mirror (2) cross-hub isolation symmetric; (4) viewer POST /api/users → 403 (accept FORBIDDEN hoặc HUB_ADMIN_REQUIRED per backend ordering Plan 02-03). Audit forensic chain D-V3.1-Phase4-E LOCKED verified runtime via SQLAlchemy async engine query `SELECT payload->>'actor_role', payload->>'actor_hub_id' FROM audit_logs WHERE action=:action ORDER BY created_at DESC LIMIT 1`. D-V3.1-Phase4-C LOCKED testcontainers in-process pattern (PostgresContainer pgvector/pgvector:pg16 + RedisContainer + asgi_lifespan LifespanManager + httpx.AsyncClient ASGITransport) carry forward Phase 1 v3.1 + v3.0 conftest existing. Pattern composite carry forward test_dep_hubs_scope.py (Plan 02-02) + test_audit_actor_metadata.py (Plan 02-04). Deviation ghi nhận: Body field `name` (Pydantic CreateUserRequest schema) — Plan template ban đầu dùng `full_name`, fixed inline. Reference: `04-02-PLAN.md` + `04-02-SUMMARY.md`.

- **Plan 04-03 closeout v3.1 SHIPPED — 4 docs atomic + git tag annotated:** STATE.md frontmatter `completed_phases: 4` + `completed_plans: 15` + `percent: 100` + `milestone_status: SHIPPED` + `milestone_shipped_date: 2026-05-24` + `next_action` reflect /gsd-complete-milestone hoặc /gsd-new-milestone v4.0 + body Phase 4 Results Summary section đầy đủ với 3 plan bullet + carry forward + R-V3.1-1/-2 mitigation final + 🎉 v3.1 SHIPPED note. REQUIREMENTS.md 2 dòng MIGRATE-XX mark `[x]` với suffix (DONE 2026-05-24 — Plan 04-XX). ROADMAP.md Phase 4 row DONE + 3 plans checklist + Milestones v3.1 ✅ Shipped 2026-05-24 + Progress table 15/15 plan 15/15 REQ-ID 100%. CLAUDE.md §6 APPEND subsection mới (file này) + bump trailing `*Cập nhật:` line reflect 🎉 v3.1 MILESTONE CLOSED 2026-05-24. Git tag annotated `v3.1` LOCAL via `git tag -a v3.1 -m "v3.1 RBAC hub_admin SHIPPED 2026-05-24..."` (KHÔNG push — operator decide qua manual `git push origin v3.1` hoặc `/gsd-complete-milestone v3.1` future command archive). D-V3.1-Phase4-F LOCKED carry forward Plan 02-05 + 03-04 v3.1 + Plan 04-07/05-06/06-05/07-05 v3.0 closeout 4 docs + git tag pattern. Reference: `04-03-PLAN.md` + `04-03-SUMMARY.md`.

**Architecture insights (Phase 4 v3.1):**

1. **Makefile shortcut `test-integration` + `test-migration`** — DRY shortcut cho operator + CI reuse; `-v --tb=short` verbose + concise traceback; testcontainers auto-spin session-scoped fixture từ conftest.py (KHÔNG cần TEST_DATABASE_URL env nếu dùng `postgres_container` recommended path).
2. **Migration verify-only with deviation honesty** — Plan 04-01 KHÔNG re-implement migration 0006 (LOCKED Plan 01-01 ship 2026-05-23 với introspect + defensive RuntimeError E-V3.1-1) — chỉ ship Makefile shortcut. Pre-existing test infra debt (2 stale assertions + 5 isolation issues) explicitly DEFERRED outside MIGRATE-01 scope. Migration semantic verified LIVE qua medinet-postgres deployment + Plan 04-02 hub_app_factory testcontainer indirect verification.
3. **Smoke E2E in-process testcontainers via `app_with_auth` + `auth_client`** — KHÔNG dùng `hub_app_factory` (FACTOR-01..03 Phase 2 v3.0 fake DSN pattern chỉ cho router mount config verify). `app_with_auth` fixture từ conftest.py (line 184-289) provides testcontainer Postgres + Redis + alembic upgrade head + LifespanManager + auto-truncate 8 bảng RESTART IDENTITY CASCADE per-test isolation. Real backend code path (Plan 02-01..04 + Plan 03-01..03) chạy thật KHÔNG mock.
4. **Audit forensic chain via SQLAlchemy async engine** — KHÔNG asyncpg.connect direct (avoid duplicate connection pool conflict). `payload->>'actor_role'` + `payload->>'actor_hub_id'` JSONB query pattern Plan 02-04 D-V3.1-Phase2-C LOCKED carry forward. Poll-based timeout 3s xử lý BackgroundTask audit emit fire-and-forget timing (memory `project_fastapi_bgtask_commit` carry forward — `get_session` commit SAU BackgroundTask, task cần dữ liệu vừa ghi phải commit tường minh).
5. **Seed 2 hub thật dmd + tdt** — per memory `project_real_hubs_deployment` (yte/duoc/hcns decorative ghost trong spec/compose; chỉ dmd + tdt thực sự seed trong DB testcontainer). UUIDs runtime generate qua `uuid.uuid4()` (KHÔNG hardcode).
6. **`GO_SEED_HASH` Argon2 cross-compat** — M2 Phase 3 ship pwdlib pin params match Go alexedwards (P6 mitigation R6). Plan 04-02 reuse `GO_SEED_HASH` constant + `GO_SEED_HASH_PLAINTEXT = "Admin@123"` từ conftest.py:132-133.
...
**🎉 v3.1 MILESTONE CLOSED 2026-05-24** — 4 phase / 15 REQ-ID / 15 plan ship · ROLE/DEP/FE/MIGRATE · proper fix bug user gán hub_admin vẫn vào central (memory `project_rbac_hub_admin_gap` 2026-05-23 trigger). 4 phase × ~3.75 plan avg = 15 plan total — small scope KHÔNG cần anti-pivot split. Linear critical path 1 → 2 → 3 → 4 ship suôn sẻ trong 1.5 ngày 2026-05-23 → 2026-05-24.

**Backward compat (Phase 4 v3.1 KHÔNG break v3.0 + Phase 1+2+3):**
- 1 file mới `test_smoke_e2e_v3_1_rbac.py` ADD — KHÔNG modify existing 200+ unit + integration test regression.
...
**Test infrastructure debt deferred (v3.2/v4.0 follow-up):**
...
### Phase 5 v3.1 Document version history pattern (VER-01..05 — 2026-05-26)

**Trigger:** v3.1 milestone re-opened 2026-05-26 (sau v3.1 SHIPPED 2026-05-24) — FE `DocumentVersionHistory.tsx` đã ship trước BE; user gặp 404 console khi mở "Lịch sử phiên bản" tab. Phase 5 backend catch-up — KHÔNG đụng FE (R-V3-2 minimal scope). 5 plan ship 5 REQ-ID VER-01..05 trong 1 ngày, 28 test (5 migration + 8 service + 10 router + 5 E2E) PASS + cluster regression 14/14 PASS in 52.91s.

- **Alembic migration 0007 introspect carry forward Plan 01-01:** `sa.inspect()` 3-STEP guard (table existence + FK precondition documents + users); downgrade defensive COUNT log + DROP TABLE atomic (KHÔNG raise RuntimeError vì schema feature-additive).
- **Schema 15 cột exact match `DocumentVersionAPI` interface** (`frontend/src/services/api.ts:599-615`) — contract bridge BE → FE LOCKED. Optional fields nullable: file_hash + extractor_used + change_note + created_by.
- **Service layer 5 public API:** `snapshot`, `restore_to_version`, `list_versions`, `get_version_with_chunks`, `get_version_file_path`. 3 private helper: `_compute_file_hash` (SHA-256 hex 64-char sync), `_enforce_retention` (CTE window function ROW_NUMBER PARTITION async), `_cleanup_orphan_files` (reference-count check async).
...
- **StreamingResponse RFC 6266 percent-encoded filename** — `Content-Disposition: attachment; filename*=UTF-8''<percent-encoded>` qua `urllib.parse.quote()` cho Vietnamese filename safe (header ASCII-only requirement). FE override với `<a download="...">` attribute; BE header chính tắc cho cURL/wget/non-FE consumer.
...
- **Actor metadata derive defense in depth:** admin → `actor_hub_id=None` / hub_admin → `actor_hub_id=document.hub_id` (Plan 02-04 v3.1 pattern); else raise 403 `HUB_ADMIN_REQUIRED` ngay cả khi `assert_hub_admin_for` upstream regress.
- **Integration test pattern Plan 04-02 v3.1 carry forward** + python-docx inline `Document().add_paragraph + doc.save(tempfile_path)` (M2 dep, KHÔNG OpenAI API call test env). Use `app_with_auth` fixture (KHÔNG `hub_app_factory('central')` vì fake DSN KHÔNG apply migration). Reuse `postgres_container + redis_container + alembic_cfg + auth_client + admin_user + admin_token + _login_get_token + GO_SEED_HASH` từ conftest.py — KHÔNG redeclare. `_wait_audit_row` + `_assert_audit_version_metadata` poll helper pattern test_audit_actor_metadata.py:92 carry forward.
- **`async for get_session(): ...` + `session.commit()` tường minh TRƯỚC `break`** — vì `break` triggers GeneratorExit → except path rollback (carry forward memory `project_fastapi_bgtask_commit`). Public API only (`get_session`), KHÔNG touch `_session_factory` private symbol.
...
- **FE contract LOCKED — KHÔNG đụng frontend (R-V3-2 minimal scope):** Plan 05-01 schema 15 field exact match `DocumentVersionAPI` interface; Plan 05-03 4 endpoint URL + response envelope exact match `api.ts:268-285`; Plan 05-04 scenario 1 test verify FE typecheck `chunks: []` empty array runtime.
- **NAMING_CONVENTION constraint name double-prefix gotcha** (Plan 05-01 hit): `app/db/base.py` `NAMING_CONVENTION` template `ck_%(table_name)s_%(constraint_name)s` apply lên `CheckConstraint(name="ck_table_x")` parameter → runtime conname = `ck_table_ck_table_x` (same behavior 0006 `ck_users_ck_users_role_enum`). Resilient query qua `WHERE conname LIKE '%ck_table_x'`. Future migration với constraint: pass `name="x"` (KHÔNG `name="ck_table_x"`) để avoid double-prefix.
...
*Cập nhật: 2026-05-26 (Phase 5 v3.1 Document Version History DONE — VER-01..05 ship 5 plan trong 1 ngày: Alembic migration 0007 document_versions 15 cột match DocumentVersionAPI + service document_version_service.py 5 API dedupe-by-hash + restore append-only + retention CTE 3+2 + audit emit 2 action + router document_versions.py 4 endpoint match api.ts:268-285 + RBAC 3-layer hybrid + 28 test PASS (5 migration + 8 service + 10 router + 5 E2E) + cluster regression 14/14 PASS in 52.91s — KHÔNG break existing suite. **v3.1 SHIPPED 2026-05-24 + Phase 5 DONE 2026-05-26** — total 5 phase / 20 REQ-ID / 20 plan · ROLE/DEP/FE/MIGRATE/VER · proper fix bug user gán hub_admin vẫn vào central + version history feature catch-up (FE DocumentVersionHistory.tsx ship trước BE, user gặp 404 console). KHÔNG tạo git tag mới mặc định — D-V3.1-Phase5 discretion semver clean, v3.1 đã tag local 2026-05-24. Project: MEDWIKI. Next: /gsd-complete-milestone v3.1 (archive .planning/milestones/v3.1-rbac-hub-admin-archive/) HOẶC /gsd-new-milestone v4.0 (Production Hardening + Advanced RAG backlog per project_v3_multi_hub_split seed + HA Redis cluster + OCR Vietnamese + streaming /api/ask SSE + coverage >80% + per-resource ACL granular) HOẶC git push origin v3.1 push tag local.)*
...
<!-- Original v3.0 trailing line (preserved for history reference):
