...
- [x] **Phase 10: Hardening + Observability + Docs** ✅ COMPLETE 2026-05-21 — structlog JSON + Prometheus + integration test ≥50% + DEPLOY.md + CI workflow GitHub Actions (10-01 ✅ structlog DONE 2026-05-21 · 10-02 ✅ Prometheus DONE 2026-05-21 · 10-03 ✅ HARD-03 critical-path acceptance suite DONE 2026-05-21 · 10-04 ✅ CRIT-01 CORS split DONE 2026-05-21 · 10-05 ✅ HARD-04 docs closeout DONE 2026-05-21 · 10-06 ✅ HARD-03 CI workflow test.yml + lint.yml secret detection DONE 2026-05-21)
...
- [x] 08.3-07-PLAN.md — Gap closure (audit 2026-05-21): vá CRIT-02 (CORS double-add path-prefix) + CRIT-03 (client_id mismatch chain load+exchange+refresh) + HIGH-01 (DCR redirect_uri whitelist) + HIGH-07 (revoke RFC 7009 silent no-op) + HIGH-08 (refresh defense-in-depth) + HIGH-09 (CSRF token end-to-end login form) — 3 commit atomic, mcp_service 116/116 PASS, ruff sạch (Wave 7, MCP-01/MCP-02) ✅ 2026-05-21
- [x] 08.3-08-PLAN.md — Gap closure (audit 2026-05-21): HIGH-02 refresh token family revocation (cột family_id NULLABLE + migration ALTER back-compat DB cũ + delete_token_family với guard NULL no-op + exchange_authorization_code sinh family_id + exchange_refresh_token reuse → xoá cả chain RFC 6749 §10.4) + HIGH-03 refresh JWT concurrency lock (asyncio.Lock per access_token + re-check downstream_jwt trong critical section single-flight refresh) + Blocker #4 cleanup (wire _release_refresh_lock vào provider.revoke_token qua late import chống circular) — 3 commit atomic, mcp_service 116/116 PASS, pytest -m critical 17/17 PASS, ruff sạch (Wave 8, MCP-01/MCP-02) ✅ 2026-05-21
- [x] 08.3-09-PLAN.md — Gap closure (audit 2026-05-21): test phủ TẤT CẢ fix Plan 07+08 + 3 commit MISSING (HIGH-04 OIDC alias regression a04b928 + HIGH-05 list_hubs envelope regression 3dbb378 + HIGH-06 DNS rebinding allowed_hosts introspect THẬT regression bd6c02c) — 19 test mới (7 provider + 2 store + 3 login + 4 path_prefix + 3 server), regression mcp_service 135/135 PASS, pytest -m critical 17/17 PASS, ruff sạch toàn bộ tests/. Pure test plan — KHÔNG sửa production code. Phase 8.3 COMPLETE 9/9 (Wave 9 CUỐI, MCP-01/MCP-02) ✅ 2026-05-21

> ✅ CRIT-01 (CORS allow_origins=* cho /token /authorize /revoke) ĐÓNG ở Plan 10-04 (2026-05-21) — MultiPolicyCORSMiddleware ASGI wrapper tách metadata wildcard `*` vs sensitive whitelist origin. Phương án B (custom middleware) thay vì A (2 Starlette sub-app) — đơn giản hơn vì subdomain mode chỉ 1 app level. 143/143 mcp_service test PASS gồm CRIT-02 regression.
...
- [x] **09-02** ✅ (Wave 2, 2026-05-21): `lib.py` 343 dòng APIClient async httpx + JWT auto-refresh on 401 + preflight_check + get_eval_hub_id + upload_and_wait (60s/file race-tolerant, 415 scanned PDF → failed_unsupported) + EvalSettings dotenv-backed. `metrics.py` 129 dòng compute_retrieval_metrics (top-1/3/5 + MRR + per_query, match title.lower() case-insensitive D-10) + compute_latency_percentiles (statistics.quantiles n=100 stdlib KHÔNG numpy, p50/p95/p99/mean). `tests/test_metrics.py` 10 test (4 @critical) PASS 10/10. EVAL-02 COMPLETE. 2 task atomic (`0d0b5c1` / `3177ddc`), 4 file mới, deviation 2 Rule 1 (F541 ruff + test p95/p99 extrapolate range).
- [x] **09-03** ✅ (Wave 2 parallel, 2026-05-21): `report.py` 360 dòng EVAL.md generator 7 section (Setup + Metrics + Per-Query Diff + Latency + Conclusion + Recommendations + Defer) qua tabulate `tablefmt='github'` + `gate_verdict(top_3)` dict 3 field (≥0.75 PASS exit 0 / 0.60-0.75 FAIL exit 1 / <0.60 trigger_e5 STOP M2b PROJECT EXIT E5) + CLI 3 exit code (0 PASS / 1 FAIL / 2 missing file). `tests/test_report.py` 257 dòng 17 unit test (3 @critical + 14 standard) PASS 17/17. EVAL-03 COMPLETE. Regression toàn eval/ 27/27 PASS. KHÔNG depend `eval/lib.py` → parallel với 09-02. 2 task atomic (`f9f1e48` / `dbd58d1`), 2 file mới, deviation 2 Rule 3 (EN DASH → HYPHEN-MINUS cho ruff RUF + I001 import order auto-fix).
- [x] **09-04** ✅ (Wave 3, 2026-05-21): `run_eval.py` 337 dòng orchestrator end-to-end 8-step (preflight → resolve hub → cleanup → login admin + upload 10 file → settle 2s cocoindex → run 12 query → metrics → write results.json + EVAL.md + exit verdict) + CLI args đủ 5 (--mock-embed/--skip-cleanup/--top-k/--output/--eval-md) + env switch `EVAL_MOCK_EMBED=1` or-logic flag. `cleanup.py` 215 dòng mixed strategy 3 layer (API DELETE cocoindex auto-tombstone + Postgres DELETE defensive + Redis DEL pattern search:*/hub:*:invalidate/rate_limit:*) + CLI --skip-* per layer + idempotent. `Makefile` thêm 8 target eval-* (install/seed/clean/smoke/all/report/readme/restore) + TAB-indent verified byte-level + help ASCII-safe. `eval/README.md` mở rộng 53 → 249 dòng 8 section (tiền điều kiện OpenAI tier paid + cài đặt + workflow 5 bước + cấu trúc + troubleshooting 9 case + reproducibility + tài liệu + Makefile targets table). EVAL-02 + EVAL-04 COMPLETE. Framework end-to-end runnable: `make eval-smoke` (Wave 1 mock < 60s) + `make eval-all` (Wave 4 gate verdict thật yêu cầu OPENAI_API_KEY). Regression toàn eval/ 27/27 PASS (10 metrics + 17 report). 3 task atomic (`f21cec7` cleanup.py / `6631301` run_eval.py / `a132d85` README + Makefile), 4 file (2 mới + 2 modified). Deviation 2 Rule 3 (UP017 datetime.UTC alias + BLE001 narrow except 5 nhóm thay Exception broad).
- [x] **09-05** ✅ (Wave 3 cuối, 2026-05-21): pytest smoke regression `api/tests/integration/test_eval_pipeline.py` 573 dòng (≥120 plan) — 3 critical test (upload_search_pipeline + scanned_pdf_failed_unsupported + hub_isolation_no_leak) với mock embed (1536-dim deterministic SHA-256) + helper inline `_upload_file` + `_poll_until_terminal` + `_search_query` + `_reconcile_status` (Phase 4 race fix). Pattern khớp test_ingest_e2e.py Phase 4 Plan 04-06: override `app_with_auth` per-file + `_cocoindex_env` session-scoped (DEF-05-01 cocoindex 1.0.3 Environment singleton). `conftest_eval.py` 203 dòng (≥50 plan): 3 fixture additive — `mock_litellm_embed` deterministic SHA-256 + `eval_hub_seeded` INSERT qua SQLAlchemy async engine schema khớp migration 0003 + `eval_admin_login` alias. CI gate `pytest -m critical api/tests/integration/test_eval_pipeline.py` chạy < 60s KHÔNG cần OPENAI_API_KEY. KHÔNG measure verdict ≥75% top-3 — đó là track standalone `make eval-all` Wave 4 HUMAN UAT. Module-level `assert callable(upload_and_wait)` verify eval.lib import reachable qua sys.path inject Hub_All/ (Plan 09-02 dependency check). 2 task atomic (`81e83ee` conftest_eval.py / `717b53b` test_eval_pipeline.py), 2 file mới + 0 modified, 776 dòng tổng. Deviation 5 auto-fix: 2 Rule 1 (F401 unused litellm + monkeypatch path sai `app.rag.scanned_detector` → đúng `app.services.file_extract` + `app.services.documents_service`); 3 Rule 2 (eval_hub_seeded SQL thiếu cột migration 0003 NOT NULL → bổ sung 10 cột khớp _insert_hub helper conftest; signature `db_session: AsyncSession` không tồn tại → đổi `app_with_auth: Any` + `get_engine()`; fixture `app_with_cocoindex` không tồn tại → override pattern test_ingest_e2e.py app_with_auth shadow + _cocoindex_env session + helper _reconcile_status race fix). EVAL-04 COMPLETE.
...
- [x] **10-04** ✅ (Wave 2, 2026-05-21): MultiPolicyCORSMiddleware đóng CRIT-01 Phase 8.3 audit defer — tách 2 CORS policy theo path: metadata (`/.well-known/*`) → ACAO `*` (RFC 8414 §3.1) vs sensitive (`/token, /authorize, /revoke, /register, /mcp[/*]`) → echo whitelist origin từ `mcp_oauth_sensitive_allowed_origins` (default 4 origin: claude.ai + inspector.modelcontextprotocol.io + localhost:6274 + 127.0.0.1:6274; env override comma-separated). ASGI wrapper xài chung 2 mode deploy (subdomain CORS NGOÀI _BasicAuthFormShim; path-prefix CORS giữa _BasicAuthFormShim và wrapper). Custom OPTIONS preflight handler trong middleware (Starlette Route default KHÔNG có OPTIONS handler cho metadata/transport). Existing-header guard skip inject nếu SDK MetadataHandler đã set (CRIT-02 regression). 2 commit atomic (RED `b414188` + GREEN `82720db`): test_cors_split_policy.py 348 dòng 8 case PASS + 143/143 mcp_service test PASS, CRIT-02 regression test_cors_no_duplicate_in_path_prefix_mode PASS. Deviation 3 auto-fix (Rule 1 ×2 existing-header guard + path-prefix variant cover; Rule 3 validation_alias + Annotated[NoDecode] cho env CSV parse). CRIT-01 ĐÓNG.
- [x] **10-05** ✅ (Wave 3, 2026-05-21): HARD-04 docs closeout — README.md mới 140 dòng (stack overview Python + quickstart + test + eval + observability + milestone status) + DEPLOY.md mới 314 dòng (7 section: prerequisites + quickstart + .env config + backup/restore 6 artifact + observability + security checklist 12 mục + M2 closeout summary) + api/.env.example mở rộng (+4 key thiếu: COCOINDEX_LMDB_PATH + 2 RATE_LIMIT + WATCHDOG_TIMEOUT + AES_KEY placeholder tường minh) + mcp_service/.env.example bổ sung (MCP_PATH_PREFIX + MCP_OAUTH_SENSITIVE_ALLOWED_ORIGINS Plan 10-04 4 origin default + MCP_INTERNAL_TOKEN) + Hub_All/CLAUDE.md section 6 mới "M2 closeout — Pivot tới v3.0 Multi-Hub Split" với 4 D-V3 LOCKED 2026-05-21 (multi-DB cùng instance + chunks+vector sync 1 chiều + milestone-level scoping + M2 closeout precondition) + 4 GA-V3 open question liệt kê tường minh + Reference link seed v3.0 + Hub_All/.planning/CONVENTIONS.md section 5 thêm note "Plan 10-01 status" actionable convention (rule "mọi log mới PHẢI dùng structlog.get_logger" + defer migrate cũ v4.0). 2 commit atomic (`c4666a8` Task 1 + `ebf3112` Task 2), 3 file mới + 4 file modified, ~520 dòng tổng. Backup script HARD-04 đúng spec: `pg_dump --schema=public --schema=cocoindex medinet_central > backup.sql` + `pg_dump medinet_cocoindex > backup_cocoindex.sql` + tarball LMDB + file_store + OAuth state + JWT keys + retention 30 ngày. Verify automated 100% PASS (4 file tồn tại + DEPLOY content + 13/13 api required key + 9/9 mcp_service required key + no secret thật). 0 Rule 1/2/3 auto-fix; 2 Enhancement (DEPLOY 7 section thay 5 + backup 6 artifact thay 2 DB). T-10-05-01/02/03 mitigate ĐẠT. HARD-04 COMPLETE. Decisions: D-10-05-A 7 section thay 5 (audit trail prod); D-10-05-B README ngắn 140 dòng KHÔNG duplicate DEPLOY; D-10-05-C backup 6 artifact thay 2 DB + cite $6.50/100K chunks; D-10-05-D AES_KEY placeholder tường minh CHANGEME_32_BYTES_BASE64_44_CHARS=; D-10-05-E MCP issuer prod-realistic https://mcp.medinet.vn/mcp; D-10-05-F 4 GA-V3 đầy đủ trong CLAUDE.md; D-10-05-G CONVENTIONS Plan 10-01 actionable convention.
- [x] **10-06** ✅ (Wave 4 cuối M2, 2026-05-21): GitHub Actions CI workflow HARD-03 + secret detection HARD-04. `.github/workflows/test.yml` 87 dòng 7 step required (Checkout + Setup Python 3.12 + Setup uv enable-cache + Sync deps `uv sync --extra dev --frozen` + Ruff check + Mypy --strict liệt kê path tường minh app/auth+middleware+observability+5 file service/repo/config + Pytest -m critical -m integration với 10 cov flag --cov=app.auth+app.routers.{auth,hubs,documents,search,ask}+app.services.{documents,search,ask}_service+app.repositories.hub_isolation + `--cov-fail-under=50` hardcode YAML KHÔNG env override + upload coverage.xml artifact retention 30 ngày). Trigger push + pull_request branch main, concurrency `${{ github.workflow }}-${{ github.ref }}` cancel-in-progress=true tiết kiệm phút CI free tier, timeout 20 phút, working-directory job-level defaults Hub_All/api DRY pattern. testcontainers Postgres pgvector pg16 + Redis 7 spin qua Docker socket runner ubuntu-22.04 (KHÔNG `services:` block YAML pre-spin — Plan 10-03 test code dùng `PostgresContainer().start()` + `RedisContainer().start()` tự control lifecycle qua Docker SDK). Mock LiteLLM qua OPENAI_API_KEY=sk-test-placeholder + GEMINI_API_KEY=gemini-test-placeholder (Plan 10-03 Test 5 mock litellm.acompletion qua SimpleNamespace) — CI gate zero external dep. `.github/workflows/lint.yml` 58 dòng 6 step quick feedback <5 phút timeout (Checkout + Setup Python 3.12 + Setup uv + Lint api/ ruff check + ruff format --check + Lint mcp_service/ optional continue-on-error + Detect secret in .env.example HARD-04 safety grep 3 pattern OpenAI `sk-[a-zA-Z0-9]{40,}` + Gemini `AIza[a-zA-Z0-9]{32,}` + AWS `AKIA[A-Z0-9]{16}` trên 2 file `.env.example` — exit 1 nếu match). Plan 10-05 verify .env.example chỉ có placeholder `sk-replace-me` 10 ký tự < 40 + CHANGEME_* KHÔNG match length 40+ → CI gate KHÔNG false-positive. CONVENTIONS.md Section 1 (Test Strategy) thêm subsection "Plan 10-06 CI gate status" 26 dòng actionable note. 2 commit atomic (`cb93e42` Task 1 test.yml + `31f4f66` Task 2 lint.yml + CONVENTIONS), 2 file mới (test.yml 87 + lint.yml 58) + 1 file modified (CONVENTIONS.md +26 dòng), 171 dòng tổng. 0 deviation — verify script automated PASS đầu lần. T-10-06-01/02/03 MITIGATE ĐẠT (placeholder env + cov-fail-under hardcode), T-10-06-04 ACCEPT (testcontainers ổn định ubuntu-22.04). **HARD-03 acceptance line "Integration test ≥50% critical path coverage chạy CI GitHub Actions" ĐÓNG.** KHÔNG push remote tại Plan 10-06 (defer trigger user khi đóng M2 milestone sau /gsd-complete-milestone v2.0 + retrospective, branch protection setup defer v4.0 cần admin permission). Decisions: D-10-06-A testcontainers self-managed Docker socket KHÔNG `services:` block; D-10-06-B 2 workflow tách (test 15 phút + lint <2 phút); D-10-06-C working-directory job-level defaults DRY; D-10-06-D `--cov-fail-under=50` hardcode YAML; D-10-06-E Mypy --strict liệt kê path tường minh; D-10-06-F Secret detection ở lint.yml; D-10-06-G placeholder OPENAI_API_KEY 18 ký tự < 40+3 KHÔNG match grep; D-10-06-H KHÔNG push remote defer user trigger.
...
| 10. Hardening + Observability + Docs | 6/6 | ✓ Complete (10-01 ✅ HARD-01 structlog · 10-02 ✅ HARD-02 Prometheus /metrics + middleware + instrument · 10-03 ✅ HARD-03 critical-path acceptance + coverage 57.75% · 10-04 ✅ CRIT-01 CORS split · 10-05 ✅ HARD-04 docs closeout + M2 closeout + v3.0 transition reference · 10-06 ✅ HARD-03 CI workflow GitHub Actions test.yml + lint.yml secret detection) | 2026-05-21 |

**Tổng:** 13/13 phases COMPLETE ✅ (M2a: 4/4 ✅ — Phase 1/2/3/4 done · M2b: 9/9 ✅ — Phase 5/6/7/8/8.1/8.2/8.3/9/10 done). **M2 v2.0 100% COMPLETE 38/38 REQ-ID 2026-05-21.** Phase 10 Plan 10-01 ✅ structlog + Plan 10-02 ✅ Prometheus + Plan 10-03 ✅ HARD-03 critical-path acceptance suite (5/5 PASS, coverage 57.75% ≥50% gate) + Plan 10-04 ✅ CRIT-01 CORS + Plan 10-05 ✅ HARD-04 docs + Plan 10-06 ✅ HARD-03 CI workflow GitHub Actions DONE 2026-05-21. M2a EXIT GATE ✅ PASSED 2026-05-21. Tiền đề D-V3-04 (M2 closeout precondition) đạt → sẵn sàng `/gsd-complete-milestone v2.0` + `/gsd-new-milestone v3.0` Multi-Hub Split (4 D-V3 LOCKED + 4 GA-V3 open question chốt ở `/gsd-discuss-milestone v3.0`).
