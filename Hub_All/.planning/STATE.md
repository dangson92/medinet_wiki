...
1. **(Recommended) `/gsd-discuss-phase 6`** — System Settings Sync (SETTINGS-01..04 — `rag_config` HTTP pull + Redis pub/sub invalidate `settings:invalidate` channel + `api_keys` verify proxy + `hub_registry` read-only hub con). GA-V3-B chốt ở discuss-phase: HTTP pull (khuyến nghị seed) vs push webhook vs env var local + cache TTL 60s default vs 5min low-change vs adaptive + pub/sub fallback nếu Redis xuống. Depends Phase 3 only — parallel-able với Phase 7 prep.
