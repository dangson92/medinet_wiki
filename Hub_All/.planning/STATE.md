...
status: "Phase 5 DONE 2026-05-23 ✅ — Reverse Proxy + Frontend Subpath 6 plan ship PROXY-01..04. Caddy wiki block (`{$WIKI_PUBLIC_DOMAIN}` server parallel MCP block Phase 8.3 carry forward) + `path_regexp hub_api ^/({$HUBS_ALLOWLIST_REGEX:yte|duoc|hcns})/api/(.*)$` + `uri strip_prefix /{re.hub_api.1}` + `reverse_proxy http://python-api-{re.hub_api.1}:8080` (port STATIC :8080 per Pitfall 1) + central `/api/*` no-strip + `/.well-known/*` JWKS pass + static SPA `try_files {path} /index.html` (D-V3-Phase5-A1/A2 LOCKED). Frontend 1-build runtime detect `window.location.pathname.split('/').filter(Boolean)[0]` + KNOWN_HUBS `window.__HUB_CONFIG__.allowlist` fallback hardcode `['yte','duoc','hcns']` + react-router `BrowserRouter basename={APP_BASE}` auto-prepend (D-V3-Phase5-B1/B3). Per-hub branding `frontend/src/branding/` Vite glob registry 4 hub initial (central indigo / yte emerald / duoc sky / hcns amber) + theme color inline CSS variable `--hub-theme` + WCAG 1.4.11 mitigation `getContrastTextColor()` hcns amber slate-900 threshold 0.6 (D-V3-Phase5-D1/D2/D3). Login.tsx 4 state machine (A/B/C/D per UI-SPEC §2) + T-5-04 strict ?return validation 4-layer + W-05 origin-not-host + window.location.replace cross-prefix redirect (D-V3-Phase5-B4/C1). Layout.tsx sidebar header logo bg + title swap module-level `getBranding(CURRENT_HUB)` + hover ring themeColor (UI-SPEC §3 LOCKED scope — chỉ sidebar header touch, KHÔNG cascade tất cả component R-V3-2 mitigation). api.crossHubSearch ABSOLUTE path `/api/search/cross-hub` qua `requestAbsolute<T>` helper bypass `${API_BASE}` prefix (D-V3-Phase4-D3 carry forward) + tryRefresh explicit `redirect: 'follow'` audit B-02 fix (D-V3-Phase5-C4). hub-add.sh 9-step pipeline extend FACTOR-04 (Step 8 sed-edit `.env` HUBS_ALLOWLIST/REGEX atomic tmp+mv + Step 9 PRE-validate caddy validate + caddy reload zero-downtime + smoke curl warn-only + dev pre-up tolerance) (D-V3-Phase5-A3 + T-5-05 Plan 02-05 carry forward). D6 EXPIRED formally CLAUDE.md §3 (PROXY-03). vitest Wave 0 infra (vitest@^2 + @testing-library/react@^16 + jest-dom + jsdom) + 5 new test file 37/37 vitest PASS full suite (6 Login + 4 Layout + 11 api + 3 App + 13 branding). R-V3-2 mitigation manual smoke 4 hub × 11 trang COMPAT-01 checkpoint Plan 05-06 Task 5b smoke SKIP pre-resolved per --auto chain + v3.0-b precedent (Plan 03-05 + 04-07) — defer Phase 7 MIGRATE-05 full E2E (3 hub + central golden path + JWT SSO live + cross-hub search live data + per-hub branding visual diff). Evidence chain Phase 5 in-process: 37/37 vitest PASS + `docker compose config --quiet` exit 0 + `caddy validate` exit 0 + `bash -n hub-add.sh` exit 0 + 36+5 acceptance grep PASS cover semantic PROXY-01..04. Backward compat M2 KHÔNG break — endpoint URL/envelope shape `{success, data, error, meta}` UNCHANGED + 11 trang nội dung React preserve (chỉ Login + Layout sidebar header touch) + localStorage same-origin SSO carry forward + 307 backend redirect Plan 03-04 failsafe Layer 2. 7 STRIDE threat coverage T-5-01..07 (6 mitigate + 1 accept localStorage XSS defer v4.0). v3.0-b 2/4 phase DONE (Phase 4+5 — 28/~32 plan ≈ 88%). Next: /gsd-discuss-phase 6 System Settings Sync (SETTINGS-01..04 — rag_config HTTP pull TTL + Redis pub/sub invalidate + api_keys proxy + hub_registry read-only — GA-V3-B chốt)."
last_updated: "2026-05-23T02:15:00.000Z"
phase_6_status: "IN-PROGRESS 2026-05-23 ⏳ — 3/5 plan ship (Plan 06-01 Wave 1 + Plan 06-02 Wave 2 + Plan 06-03 Wave 3 DONE). Wave 3 REQUEST-RESPONSE INTEGRATION ship 4 file source modify + 1 schema mới + 4 file test mới — refactor `require_api_key` dependency branch verify path theo `settings.hub_name` (central=ApiKeyService.verify_key M2 local AES-GCM carry forward / hub con=`request.app.state.api_key_verify_client.verify` HTTP proxy + Redis cache 60s Plan 06-02) với 503 `APIKEY_VERIFY_CLIENT_UNAVAILABLE` fail-loud nếu Wave 4 lifespan chưa init (D-V3-Phase6-A LOCKED + R-V3-6 LOW mitigation), tạo `require_internal_auth` async dependency mới ở `api/app/auth/dependencies.py` với `hmac.compare_digest` constant-time compare header `X-Internal-Auth == settings.settings_proxy_secret` (T-06-03-01 timing attack mitigation 128-bit entropy enforce qua Plan 06-01 Settings validator 32 char min) → 401 `INTERNAL_AUTH_FAIL` envelope (D-V3-Phase6-D LOCKED), extend `update_rag_config` PUT thêm `request: Request` param + publish `settings:invalidate` channel JSON payload `{config_key:'rag_config', hub:'*', timestamp:<unix>}` best-effort fail-open try/except + log.warning sau service.update_config() success (T-06-03-04 DoS mitigation + carry forward search_cache.py M2 fail-open pattern), và thêm endpoint MỚI `POST /api/api-keys/verify` central-only mount (FACTOR-02 carry forward main.py:704 conditional include) với `Depends(require_internal_auth)` gate + `VerifyApiKeyRequest` Pydantic schema body `{api_key: str min_length=1 max_length=256}` + return raw dict `{valid: bool, principal: dict|null}` (pattern song song /api/rag-config M2 raw dict — hub con ApiKeyVerifyClient Plan 06-02 parse trực tiếp). 4 atomic commit Plan 06-03 (a5d9ffb RED task1 + c26e02d GREEN task1 + 5e807ac RED task2 + 803a0a3 GREEN task2). 0 deviation — Plan executed exactly as written. 22 unit test PASS (6 require_api_key branch + 6 require_internal_auth + 4 rag_config publish + 6 api_keys/verify endpoint = Plan target exact match) + 452/452 unit Phase 1..6 regression PASS (430 baseline + 22 mới) + 50/50 cluster regression (`-k 'auth or rag_config or api_keys or sso'`) + 19/19 integration regression (15 factor_hub_scoped + 4 rate_limit consumer require_api_key) + ruff + mypy --strict clean trên 5 source file + 4 test file. Next: Plan 06-04 Wave 4 lifespan integration (BLOCKING depend Wave 1+2+3) — init `app.state.{rag,hub_registry,api_key_verify}_client` + spawn settings_subscriber_task + SETTINGS_SKIP_FETCH=1 escape hatch + integration test ASGI LifespanManager."
progress:
  total_phases: 7
  completed_phases: 5
  total_plans: 33  # 28 (Phase 1..5) + 5 (Phase 6 plans 06-01..06-05)
  completed_plans: 31  # 28 + 3 (Plan 06-01 + 06-02 + 06-03)
  percent: 94  # 31/33 ≈ 94% (incremental Wave 1 + Wave 2 + Wave 3 + 2 plan còn lại Phase 6)
...
- **Phase:** 6 — System Settings Sync ⏳ **IN-PROGRESS 2026-05-23 — Plan 06-01 Wave 1 + Plan 06-02 Wave 2 + Plan 06-03 Wave 3 DONE (3/5 plan)**
- **Plan:** 06-03 ✅ DONE — Wave 3 REQUEST-RESPONSE INTEGRATION (require_api_key branch settings.hub_name central=local M2 / hub con=app.state.api_key_verify_client proxy 503 fail-loud + require_internal_auth dep hmac.compare_digest constant-time + update_rag_config publish settings:invalidate best-effort fail-open + POST /api/api-keys/verify endpoint mới central-only mount FACTOR-02 + VerifyApiKeyRequest Pydantic schema). 22 unit test PASS (6 require_api_key + 6 require_internal_auth + 4 rag_config publish + 6 api_keys/verify — Plan target exact match) + 452/452 Phase 1..6 unit regression + 50/50 cluster regression (auth/rag_config/api_keys/sso) + 19/19 integration regression (factor_hub_scoped + rate_limit) + ruff + mypy --strict clean. 4 atomic commit (a5d9ffb RED task1 + c26e02d GREEN task1 + 5e807ac RED task2 + 803a0a3 GREEN task2). 0 deviation — Plan executed exactly as written.
- **Next:** Plan 06-04 Wave 4 — lifespan integration BLOCKING (depend Wave 1+2+3). Init `app.state.rag_config_client` + `app.state.hub_registry_client` + `app.state.api_key_verify_client` + spawn `settings_subscriber_task` ở lifespan hub con block + shutdown graceful + `SETTINGS_SKIP_FETCH=1` escape hatch. Integration test ASGI LifespanManager verify `app.state.<X>_client` populated + subscriber task running.
...
1. **(Recommended) `/gsd-execute-phase 6 --auto --chain`** — Wave 1 BLOCKING (06-01 scaffold infra) → Wave 2 (06-02 client + subscriber) → Wave 3 (06-03 refactor + extend) → Wave 4 BLOCKING (06-04 lifespan + integration test) → Wave 5 closeout (06-05 docs + Task 5b smoke auto-fallback per --auto chain + v3.0-b precedent). Expected: 4/4 REQ SETTINGS-01..04 closed, 4/4 D-V3-Phase6 consumed, evidence chain in-process unit + ASGI integration test cover semantic.
2. (Optional) `/gsd-code-review 5` — advisory code review trên ~13 commits Phase 5 (workflow.code_review gate), phủ 6 plan ship PROXY-01..04 + 16 decision LOCKED D-V3-Phase5-A1..D4 + 7 STRIDE threat T-5-01..07 + FACTOR-04 hub-add.sh 9-step extend.
3. (Optional) `/gsd-verify-work 5` — manual UAT 4 SC nếu user muốn extra verify ngoài automated test (Plan 05-06 Task 5b smoke 4 hub × 11 trang SKIP pre-resolved, sẽ verify ở Phase 7 MIGRATE-05 runtime smoke E2E live 3 hub + central + golden path + per-hub branding visual diff).
4. (Optional) **Parallel `/gsd-discuss-phase 7`** — Migration + Smoke E2E (MIGRATE-01..05 — pg_dump per hub_id snapshot + restore blue/green + truncate central skeleton + MCP re-point central + smoke E2E 3 hub con + central PASS). Depends Phase 1-6; GA-V3-D part 2 chốt strategy.
