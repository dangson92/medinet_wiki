...
last_updated: "2026-05-23T01:45:00.000Z"
phase_6_status: "IN-PROGRESS 2026-05-23 ⏳ — 2/5 plan ship (Plan 06-01 Wave 1 + Plan 06-02 Wave 2 DONE). Module `api/app/settings_sync/` đầy đủ business logic core 5 file (Wave 1 scaffold: __init__.py + keys.py + metrics.py — Plan 06-01; Wave 2 core: client.py + subscriber.py — Plan 06-02). Plan 06-02 ship 3 HTTP client class (RagConfigClient + HubRegistryClient + ApiKeyVerifyClient) với httpx async (Timeout connect=2s/read=5s initial + connect=5s/read=10s refresh) + Redis cache TTL hybrid (60s rag_config + 300s hub_registry singleton + 60s apikey) + Prometheus emit (6 collector W7 fix label hub_name bounded) + fail-loud lifecycle (SettingsUnavailableError on boot fail — R-V3-6 LOW mitigation), và `settings_subscriber_loop` asyncio task subscribe single channel `settings:invalidate` + Pydantic InvalidateMessage Literal enum 3 config_key (T-06-02-03 Tampering mitigation) + 3 flush branch dispatcher (rag_config hub_match + hub_registry singleton + apikey full SCAN+DEL) + reconnect retry generic Exception (best-effort fail-open T-06-02-05 DoS) + CancelledError graceful re-raise (lifespan task.cancel pattern). 4 atomic commit Plan 06-02 (9701fda RED client + 2890973 GREEN client + 102b512 RED subscriber + 17ff523 GREEN subscriber). 3 Rule 1 deviation auto-fix: (1) httpx.Timeout 0.28.1 yêu cầu default scalar + connect/read override (httpx API change vs Phase 3 pattern); (2) pytest-asyncio + raise CancelledError trong async generator gây hang infinite reconnect → refactor async gen yield-drain + asyncio.wait_for(timeout=2s) test pattern + asyncio.create_task + task.cancel() cho test graceful shutdown; (3) C901 complexity settings_subscriber_loop 14 > 10 → extract _process_message + _subscribe_and_listen helper. 22 unit test PASS (11 client + 11 subscriber) + 430/430 Phase 1..5 + Wave 1 + Wave 2 regression PASS + ruff + mypy --strict clean. Next: Plan 06-03 Wave 3 (require_api_key branch + require_internal_auth + update_rag_config publish + /api/api-keys/verify endpoint)."
progress:
  total_phases: 7
  completed_phases: 5
  total_plans: 33  # 28 (Phase 1..5) + 5 (Phase 6 plans 06-01..06-05)
  completed_plans: 30  # 28 + 2 (Plan 06-01 + 06-02)
  percent: 91  # 30/33 ≈ 91% (incremental Wave 1 + Wave 2 + 3 plan còn lại Phase 6)
...
- **Phase:** 6 — System Settings Sync ⏳ **IN-PROGRESS 2026-05-23 — Plan 06-01 Wave 1 + Plan 06-02 Wave 2 DONE (2/5 plan)**
- **Plan:** 06-02 ✅ DONE — Wave 2 BUSINESS LOGIC CORE (client.py 3 HTTP client class + SettingsUnavailableError + subscriber.py settings_subscriber_loop + InvalidateMessage Pydantic schema + 3 config_key flush branch dispatcher + reconnect retry + CancelledError graceful). 22 unit test PASS (11 client + 11 subscriber) + 430/430 Phase 1..5 + Wave 1 + Wave 2 regression PASS + ruff + mypy --strict clean. 4 atomic commit (9701fda RED client + 2890973 GREEN client + 102b512 RED subscriber + 17ff523 GREEN subscriber). 3 Rule 1 deviation auto-fix: (1) httpx.Timeout 0.28.1 4-param structure (default scalar + connect/read override); (2) pytest-asyncio hang khi raise CancelledError trong async gen → asyncio.wait_for timeout pattern + task.cancel() cho graceful shutdown test; (3) C901 complexity 14 → extract _process_message + _subscribe_and_listen helper.
- **Next:** Plan 06-03 Wave 3 — `require_api_key` refactor branch verify path (central=local M2 / hub con=ApiKeyVerifyClient proxy) + `require_internal_auth` dependency mới (hmac.compare_digest header check) + `POST /api/api-keys/verify` endpoint central + `PUT /api/rag-config` extend publish `settings:invalidate` best-effort fail-open. Depend Wave 1 Settings.settings_proxy_secret + Wave 2 ApiKeyVerifyClient + InvalidateMessage.
