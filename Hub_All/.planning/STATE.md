...
progress:
  total_phases: 7
  completed_phases: 6  # Phase 7 in-progress (4/5 plan done)
  total_plans: 38  # 33 (Phase 1..6) + 5 (Phase 7 plans 07-01..07-05)
  completed_plans: 37  # 33 + 4 (Phase 7 Plan 07-01 + 07-02 + 07-03 + 07-04 ship)
  percent: 97  # 37/38 ≈ 97% (Phase 7 còn 1 plan 07-05 closeout)
...
- **Phase:** 7 — Migration + Smoke E2E 🔄 **IN PROGRESS — Plan 07-01 + 07-02 + 07-03 + 07-04 DONE 2026-05-23 (Wave 1+2+3 BLOCKING + Wave 3 parallel MIGRATE-01+02+03+04 ship)**
- **Plan:** 4/5 plans complete (07-01 + 07-02 + 07-03 + 07-04) cho MIGRATE-01..04 ở `.planning/phases/07-migration-smoke-e2e/`. Next: Plan 07-05 (Wave 4 closeout — `05-smoke-e2e.sh` automated 3 hub × 7-step golden path + docs CLAUDE.md + STATE.md + ROADMAP.md + REQUIREMENTS.md + README.md + milestone v3.0 CLOSED marker).
- **Status (Phase 7):** Wave 3 parallel MIGRATE-04 resolved — `mcp_service/mcp_app/config.py` Settings.api_base_url default đổi `http://localhost:8180` → `http://python-api-central:8080` + 5-line decision comment (D-V3-Phase7-C LOCKED 2026-05-23 + carry forward D-V3-02 Phase 1 + Phase 4 Plan 04-05 D-V3-Phase4-D1 cross-hub aggregate 1 SQL + T-08.2-01-T SSRF validator unchanged). `docker-compose.yml` mcp_service env block comment "re-confirm" → "CONFIRMED Phase 7 MIGRATE-04" (env value `MCP_API_BASE_URL=http://python-api-central:8080` không đổi đã đúng từ Plan 02-02 D-V3-Phase2-C). `scripts/migrate/06-mcp-smoke.md` 181 LOC manual runbook 5-step Inspector OAuth (pre-deploy verify `uv run pytest -q` expect 135 passed Phase 8.3 v2.0 baseline + Step 1 OAuth connect `wiki.medinet.vn/mcp/auth/v2/authorize` + Step 2 `search_wiki(query="vaccin", hub_id="yte")` single-hub envelope D6 verify + Step 3 `ask_wiki(query="dược ngoại")` cross-hub no hub_id citations MULTIPLE hub aggregate verify + Step 4 citation [N] resolve `GET /api/documents/<id>` + Step 5 Prometheus metrics scrape `apikey_verify_total{result=cached}` + `cross_hub_search_latency_seconds` + `sync_lag_seconds < 30` + `sync_count_drift < 0.01` + rollback procedure 5-step + reference links). D-V3-Phase7-C LOCKED invariant — MCP gọi central aggregate KHÔNG fan-out N hub. MCP tools signature `search_wiki(hub_id?)` + `ask_wiki(hub_id?)` UNCHANGED + OAuth flow Phase 8.3 v2.0 UNCHANGED + `api_key_service` AUX-02 AES-GCM UNCHANGED + `field_validator _validate_base_url` UNCHANGED (T-08.2-01-T SSRF Phase 8.3 v2.0 carry forward). Regression: `uv run pytest -q` (`mcp_service/tests/`) → **143 passed in 3.67s** (Phase 8.3 v2.0 baseline 135 + Plan 10-04 CORS split policy 8 test thêm) — 0 regression sau config.py default change. T-07-04-01..04 STRIDE 3 mitigate + 1 accept (T-07-04-03 audit M2 carry forward AUX). 19/19 acceptance grep PASS (8 Task 1 + 11 Task 2). 1 Rule 3 deviation auto-fix `uv run pytest` substitution (python -m pytest direct fail `ModuleNotFoundError: aiosqlite` → switch `uv run` per plan instruction interfaces section). SCOPE LIMIT honored — KHONG run Inspector OAuth live against real infra. Foundation Phase 7 4/5 plan resolve — Plan 07-05 closeout là plan cuối Phase 7 + milestone v3.0.

## Phase 7 Plan 07-01 + 07-02 + 07-03 + 07-04 Results Summary

| Plan | Wave | Objective | Commits | Tests |
|------|------|-----------|---------|-------|
| 07-01 | 1 BLOCKING | scripts/migrate/01-snapshot-hubs.sh pg_dump per hub_id + migrate-snapshots/ .gitignore + README 30-day retention | 2 (e057e5f feat snapshot script + 786586d docs gitignore + README) | bash -n exit 0 + 18/18 grep acceptance PASS (9 Task 1 + 9 Task 2) |
| 07-02 | 2 BLOCKING | scripts/migrate/02-restore-hub.sh psql restore 4-step + scripts/migrate/03-switch-caddy.sh VERIFY-ONLY 3-step + --rollback flag (D-V3-Phase7-B blue/green per-hub) | 2 (81256fc feat 02-restore-hub.sh psql restore + f1556cc feat 03-switch-caddy.sh verify-only post Phase 5 dynamic regex) | bash -n exit 0 cả 2 + 21/21 grep acceptance PASS (10 Task 1 + 11 Task 2) |
| 07-03 | 3 BLOCKING | scripts/migrate/04-truncate-central.sh batched DELETE 4 table central per hub_id + audit_logs INSERT TRƯỚC DELETE atomic transaction + dry-run default ON + chunks PRESERVED D-V3-02 invariant strict (MIGRATE-03) | 1 (5ade696 feat 04-truncate-central.sh batched DELETE + audit INSERT + dry-run default) | bash -n exit 0 + 15/15 grep acceptance PASS + strict invariant DELETE FROM chunks = 0 + 10 D-V3-02 reference |
| 07-04 | 3 parallel | mcp_service/mcp_app/config.py default re-point http://python-api-central:8080 (D-V3-Phase7-C) + docker-compose.yml comment CONFIRMED + scripts/migrate/06-mcp-smoke.md 5-step Inspector OAuth runbook (MIGRATE-04) | 2 (f3934fe feat config + compose comment + 7fdac43 docs 06-mcp-smoke.md runbook) | uv run pytest -q (mcp_service): 143 passed (≥135 baseline Phase 8.3 v2.0 + Plan 10-04 CORS 8 thêm) + 19/19 grep acceptance PASS (8 Task 1 + 11 Task 2) |

**Phase 7 Plan 07-04 deliverable summary:**
- `Hub_All/mcp_service/mcp_app/config.py` (line 33-38) — Settings.api_base_url default đổi `http://localhost:8180` → `http://python-api-central:8080` + 5-line decision comment (D-V3-Phase7-C LOCKED 2026-05-23 + carry forward D-V3-02 Phase 1 + Phase 4 Plan 04-05 D-V3-Phase4-D1 cross-hub aggregate 1 SQL + T-08.2-01-T SSRF validator unchanged); field_validator `_validate_base_url` (lines 106-119) UNCHANGED preserve.
- `Hub_All/docker-compose.yml` (line 264-268) — mcp_service env block comment update "re-confirm" → "CONFIRMED Phase 7 MIGRATE-04" + thêm "D-V3-Phase7-C LOCKED 2026-05-23" reference + "Cross-hub search 1 SQL aggregated Phase 4 Plan 04-05 carry forward" note; env value `MCP_API_BASE_URL: http://python-api-central:8080` UNCHANGED (đã đúng từ Plan 02-02 D-V3-Phase2-C).
- `Hub_All/scripts/migrate/06-mcp-smoke.md` 181 LOC manual runbook — Pre-deploy verify mandatory (`uv run pytest -q` expect 135 baseline) + 5-step Inspector OAuth smoke (Step 1 OAuth connect Discovery 200 + Step 2 search_wiki single-hub yte envelope D6 + Step 3 ask_wiki cross-hub no hub_id citations MULTIPLE hub aggregate verify D-V3-Phase4-D1 + Step 4 citation [N] resolve document detail + Step 5 Prometheus metrics scrape apikey_verify_total{result=cached} + cross_hub_search_latency_seconds + sync_lag_seconds < 30 E-V3-4 + sync_count_drift < 0.01 R-V3-1) + rollback procedure 5-step (docker compose stop + git revert + rebuild + regression + Inspector re-verify) + reference links (config.py + docker-compose lines + 07-CONTEXT.md + 07-04-PLAN.md + Phase 8.3 v2.0 archive + Phase 4 Plan 04-05 LOCKED).

**Phase 7 Plan 07-03 deliverable summary:**
- `Hub_All/scripts/migrate/04-truncate-central.sh` 269 LOC bash strict mode — Dry-run default ON safety (operator phải explicit `--apply`) + atomic transaction psql heredoc `BEGIN/COMMIT-or-ROLLBACK` wrap INSERT audit_logs TRƯỚC DELETE 4 table same transaction + jsonb_build_object payload (4 row_count COUNT subquery + hub_name + dry_run + timestamp) + audit_logs DELETE filter `action != 'migrate.truncate_hub'` preserve evidence chain + per-hub error isolation set +e wrap + FAIL_COUNT tracker + confirm prompt + --yes auto-confirm cho CI + hub validation chain T-5-05 + exit codes 0/1/2/3/4 distinct + 10 explicit D-V3-02 reference (script header comment + SQL pre/post-DELETE label + summary verify hint).

**Phase 7 Plan 07-02 deliverable summary:**
...
- `Hub_All/scripts/migrate/03-switch-caddy.sh` 132 LOC bash strict mode VERIFY-ONLY mode — per CONTEXT.md correction Phase 5 Caddyfile dynamic regex `{re.hub_api.1}` đã handle routing tự động + 3-step pipeline (docker compose ps verify container running + caddy validate Pitfall 7 mitigation + curl smoke warn-only) + `--rollback` flag stop container (Caddy fall-through file_server 404 T-07-02-04 accept) + hub name validation chain T-5-05 + exit codes 0/2/3/4.

**Phase 7 Plan 07-01 deliverable summary (carry forward):**
- `Hub_All/scripts/migrate/01-snapshot-hubs.sh` 194 LOC bash strict mode — pg_dump --data-only --no-owner --no-acl --column-inserts 5 table (chunks/documents/users/audit_logs/usage_events) --where hub_id filter cho 3 hub default (yte/duoc/hcns) HOẶC single hub arg + resolve hub_id UUID qua psql -tA SELECT id FROM hubs + sanity check grep -c '^INSERT' WARN nếu 0 + idempotent skip neu output file da ton tai + exit codes 0/1/2/3/4 distinct semantic.
- `Hub_All/migrate-snapshots/.gitignore` exclude *.sql + *.sql.gpg + *.dump + whitelist .gitignore + README.md tracked (T-07-01-02 mitigation).
- `Hub_All/migrate-snapshots/README.md` document file naming convention migrate-<hub>-<YYYY-MM-DD>.sql + generate snapshot usage + 30-day retention manual + cron 4AM + privacy/PII warn + recovery rollback procedure (DROP + hub-init + psql -f + re-smoke).
