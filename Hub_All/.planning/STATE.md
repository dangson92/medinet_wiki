...
- **Phase:** 6 — System Settings Sync ⏳ **IN-PROGRESS 2026-05-23 — Plan 06-01 Wave 1 + 06-02 Wave 2 + 06-03 Wave 3 + 06-04 Wave 4 DONE (4/5 plan)**
- **Plan:** 06-04 ✅ DONE — Wave 4 BLOCKING RUNTIME WIRING (`api/app/main.py::lifespan` hub con block init 3 client `RagConfigClient` + `HubRegistryClient` + `ApiKeyVerifyClient` từ Wave 2 với blocking `fetch_initial()` 5s + fail-loud raise → uvicorn exit 1 (D-V3-Phase6-A LOCKED + R-V3-6 LOW pattern song song Phase 3 JWKSCache) + spawn `settings_subscriber_task asyncio.create_task(settings_subscriber_loop)` subscribe `settings:invalidate` Wave 2 với guard `app.state.redis is not None AND app.state.redis_ready` (T-06-04-04 Rule 3 fix) + populate `app.state.{rag_config_client, hub_registry_client, api_key_verify_client, settings_subscriber_task}` runtime cho Wave 3 dependency consume + test-mode escape hatch `SETTINGS_SKIP_FETCH=1` env + shutdown graceful `task.cancel() + asyncio.wait_for(timeout=10s)`). 6 integration test ASGI LifespanManager PASS (skip path + populate 3 client + subscriber branch by redis_ready + shutdown state reset + boot fail-loud direct RagConfigClient test + central skip). Pubsub e2e defer Phase 7 MIGRATE-05. 452/452 unit Phase 1..6 regression + 19/19 Phase 2+5 integration regression + ruff + mypy --strict clean. 2 atomic commit (ddf28e1 feat lifespan + b8a4221 test integration test + Rule 3 fix). 3 Rule 3 deviation auto-fix inline (main.py redis_ready guard + 2 test infra conftest/test_auth_router thêm SETTINGS_SKIP_FETCH=1 env).
- **Next:** Plan 06-05 Wave 5 — closeout docs CLAUDE.md §6 + STATE.md (file này) frontmatter + Phase 6 Planning + Results Summary + REQUIREMENTS.md SETTINGS-01..04 mark [x] + ROADMAP.md Phase 6 row DONE + README.md NEW section "System Settings Sync Deploy Notes (Phase 6 v3.0)" + smoke checkpoint runtime SKIP auto-fallback per `--auto chain` mode + v3.0-b precedent (Plan 03-05 + 04-07 + 05-06).
