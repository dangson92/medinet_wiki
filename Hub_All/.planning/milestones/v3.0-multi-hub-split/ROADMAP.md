# Milestone v3.0 — Multi-Hub Split (ARCHIVE)

**Status:** ✅ SHIPPED 2026-05-23
**Phases:** 1-7 (reset numbering — D-V3-05)
**Total plans:** 38/38
**REQ-ID consumed:** 30/30 (TOPO 4 + FACTOR 4 + SSO 4 + SYNC 5 + PROXY 4 + SETTINGS 4 + MIGRATE 5)

> Archive snapshot — toàn bộ ROADMAP.md tại thời điểm v3.0 close. Working ROADMAP.md đã collapsed.
...
- ✅ **v3.0 Multi-Hub Split** — **SHIPPED 🎉 2026-05-23**, 7 phases / 38 plans / 30 REQ-ID consumed (TOPO 4 + FACTOR 4 + SSO 4 + SYNC 5 + PROXY 4 + SETTINGS 4 + MIGRATE 5), phase numbering reset về 1 (D-V3-05). v3.0-a (Phase 1-3) + v3.0-b (Phase 4-7) anti-pivot complete. Next: `/gsd-complete-milestone v3.0` archive milestone.
...
| ✅ **7** | Migration + smoke E2E 🎉 | pg_dump per hub_id; restore blue/green per-hub; truncate central skeleton (chunks PRESERVED D-V3-02); MCP re-point central aggregate; smoke E2E 3 hub + central golden path 7-step automated + Prometheus assertion; 5 bash script + 1 runbook + 1 fixture DOCX | MIGRATE-01..05 (5) | 5 | Phase 1-6 — ✅ **DONE 2026-05-23 (5 plans / ~10-15 commits)** |

---

## 🎉 v3.0 MILESTONE CLOSED 2026-05-23

**38/38 plan ship · 30/30 REQ-ID consumed · 7/7 phase complete** — v3.0 Multi-Hub Split milestone hoàn tất. v3.0-a (Phase 1-3) + v3.0-b (Phase 4-7) anti-pivot pattern hoàn tất. Next: `/gsd-complete-milestone v3.0` separate command — archive `.planning/milestones/v3.0-archive/` + reset ROADMAP.md cho v4.0 backlog (sub-hub split per `project_v3_multi_hub_split.md` seed memory 2026-05-21 + HA Redis cluster + OCR Vietnamese + streaming /api/ask + comprehensive coverage >80%).
...
- [x] 07-02-PLAN.md — `scripts/migrate/02-restore-hub.sh` psql -f restore vào medinet_hub_<HUB> blue/green (D-V3-Phase7-B) + `03-switch-caddy.sh` VERIFY-ONLY (Caddyfile dynamic regex KHÔNG sed) + --rollback flag (MIGRATE-02) — **DONE 2026-05-23** (2 commit 81256fc feat 02-restore-hub.sh psql restore 4-step 225 LOC + f1556cc feat 03-switch-caddy.sh verify-only 3-step 132 LOC; bash -n PASS cả 2 + 21 grep acceptance PASS (10 Task 1 + 11 Task 2); D-V3-Phase7-B blue/green LOCKED + T-07-02-01..05 STRIDE 3 mitigate + 2 accept; 1 Rule 1 deviation inline fix psql flag order; scope limit honored — KHONG run psql/docker/caddy runtime)
- [x] 07-03-PLAN.md — `scripts/migrate/04-truncate-central.sh` atomic BEGIN/COMMIT DELETE 4 table per hub_id + audit_logs INSERT TRƯỚC DELETE (Phase 4 W8 pattern) + dry-run DEFAULT ON safety + --apply explicit; KHÔNG xoá chunks table (D-V3-02 LOCKED) (MIGRATE-03) — **DONE 2026-05-23** (1 commit 5ade696 feat 04-truncate-central.sh 269 LOC batched DELETE + audit INSERT + dry-run default; bash -n exit 0 + 15 grep acceptance PASS + strict invariant `grep -c 'DELETE FROM chunks' = 0` verified + 10 explicit D-V3-02 reference; 3-layer defense-in-depth dry-run (default + --apply explicit + confirm prompt OR --yes bypass CI); T-07-03-01..05 STRIDE 4 mitigate + 1 mitigate accept; 1 Rule 1 deviation inline fix comment phrasing; scope limit honored — KHONG run psql DELETE runtime)
- [x] 07-04-PLAN.md — `mcp_service/mcp_app/config.py` Settings.api_base_url default re-point `http://python-api-central:8080` (D-V3-Phase7-C) + docker-compose.yml comment update CONFIRMED + `scripts/migrate/06-mcp-smoke.md` 5-step Inspector OAuth runbook + 135/135 mcp_service test regression mandatory (MIGRATE-04) — **DONE 2026-05-23** (2 commit f3934fe feat config.py default + docker-compose comment + 7fdac43 docs 06-mcp-smoke.md runbook 181 LOC; mcp_service/mcp_app/config.py 1-line default change + 5-line decision comment D-V3-Phase7-C + Phase 4 Plan 04-05 cross-hub aggregate 1 SQL + T-08.2-01-T SSRF validator unchanged; docker-compose.yml mcp_service env comment "re-confirm" → "CONFIRMED Phase 7 MIGRATE-04" (env value MCP_API_BASE_URL unchanged Phase 2 D-V3-Phase2-C); 06-mcp-smoke.md 5-step manual Inspector OAuth runbook (pre-deploy uv run pytest -q + Step 1 OAuth connect + Step 2 search_wiki single-hub + Step 3 ask_wiki cross-hub + Step 4 citation resolve + Step 5 Prometheus metrics scrape + rollback procedure 5-step); 143 passed regression (Phase 8.3 v2.0 baseline 135 + Plan 10-04 CORS split 8 thêm) — 0 regression; T-07-04-01..04 STRIDE 3 mitigate + 1 accept (T-07-04-03 M2 audit AUX carry forward); MCP tools signature LOCKED unchanged + OAuth flow Phase 8.3 v2.0 UNCHANGED + api_key_service AUX-02 AES-GCM UNCHANGED; 19/19 acceptance grep PASS (8 Task 1 + 11 Task 2); 1 Rule 3 deviation auto-fix uv run pytest substitution (verify-only runtime command); SCOPE LIMIT honored — KHONG run Inspector OAuth flow live against real infra (runbook documents manual operator steps)
- [x] 07-05-PLAN.md — `scripts/migrate/05-smoke-e2e.sh` automated 3 hub × 7-step golden path (login + upload + poll + search local + cross-hub + ask + citation [N]) + Prometheus assertion p95 < 1.5s/sync_lag < 30s/cache_hit/drift < 1% + fixture sample-document.docx + 5-doc closeout + v3.0 milestone CLOSED 🎉 + manual visual smoke checkpoint advisory KHÔNG blocking (D-V3-Phase7-D) (MIGRATE-05) — **DONE 2026-05-23** (4 commits: 78aad9a feat fixture DOCX + python-docx generator + 826c93d feat 05-smoke-e2e.sh 271 LOC automated 3 hub × 7-step + 8916701 docs CLAUDE.md §6 Phase 7 pattern subsection + 210253d docs STATE.md + this commit ROADMAP + REQUIREMENTS + README); scripts/migrate/05-smoke-e2e.sh 271 LOC bash strict mode automated golden path qua curl + jq envelope D6 + Prometheus assertion post-loop (apikey_verify_total{result=cached} > 0 + sync_count_drift < 0.01 + cross_hub_search_latency_seconds histogram populated); fixture sample-document.docx 37KB Vietnamese y tế (vaccin + dược keyword cho search/ask test) + generate-sample.py python-docx reproducible script; 5-doc closeout (CLAUDE.md + STATE.md + REQUIREMENTS.md + ROADMAP.md + README.md) reflect Phase 7 DONE + v3.0 MILESTONE CLOSED 100% 38/38 plan; manual visual smoke 4 hub × 11 trang React M2 COMPAT-01 checkpoint:human-action gate=advisory KHÔNG blocking per D-V3-Phase7-D LOCKED — auto-fallback `skip smoke` per --auto chain + v3.0-b precedent (Plan 03-05 + 04-07 + 05-06 + 06-05 pre-resolved skip pattern); bash -n exit 0 + 12 grep acceptance PASS + python-docx verify 8 paragraphs DOCX; T-07-05-01..05 STRIDE 2 mitigate + 3 accept (auth gates + DoS smoke + visual smoke advisory accept); SCOPE LIMIT honored — KHÔNG run smoke against live infra, defer ops handover post-milestone-close deployment runbook.

**Decisions (chốt 2026-05-23 theo planner seed defaults Auto Mode `--auto --chain`, 4 D-V3-Phase7-A..D LOCKED):**
- D-V3-Phase7-A: `pg_dump --where` option a (REJECT cocoindex replay option b backup-only) — speed + schema match + vector preserve + failure backup reserved.
- D-V3-Phase7-B: Blue/green per-hub zero-downtime (REJECT full downtime weekend / live replication) — Caddy reload ~50-100ms negligible + snapshot 30-day rollback path enable.
- D-V3-Phase7-C: MCP re-point central aggregate (REJECT fan-out N hub) — carry forward D-V3-02 + Phase 4 Plan 04-05 cross-hub 1 SQL + Phase 8.3 OAuth stable.
- D-V3-Phase7-D: Automated smoke `05-smoke-e2e.sh` mandatory BLOCKING + human visual smoke supplement advisory KHÔNG blocking — Phase 7 cuối, KHÔNG defer; visual regression log v3.1 follow-up issue.
...
| **v3.0 Multi-Hub Split** | **7** | **38/38** | **30/30** | ✅ **Shipped 🎉 2026-05-23 — All 7/7 phases complete · 38/38 plans · 30/30 REQ-ID consumed (TOPO 4 + FACTOR 4 + SSO 4 + SYNC 5 + PROXY 4 + SETTINGS 4 + MIGRATE 5) · v3.0-a (Phase 1-3) + v3.0-b (Phase 4-7) anti-pivot pattern hoàn tất. Next: /gsd-complete-milestone v3.0 archive milestone.** | 2026-05-23 |
...
*Last updated: 2026-05-23 sau `/gsd-execute-phase 7` Plan 07-05 closeout — 🎉 v3.0 MILESTONE CLOSED 2026-05-23 — Phase 7 Migration + Smoke E2E DONE 5 plans (Wave 1 BLOCKING + Wave 2 BLOCKING + Wave 3 BLOCKING + Wave 3 parallel + Wave 5 closeout). 4 D-V3-Phase7-A/B/C/D LOCKED consumed; pg_dump --where option a snapshot per hub_id + blue/green per-hub restore + truncate central (chunks PRESERVED D-V3-02) + MCP re-point central aggregate + automated 3 hub × 7-step smoke E2E golden path + Prometheus assertion + manual visual smoke advisory KHÔNG blocking. 5 bash script `scripts/migrate/` ~1091 LOC + 1 runbook MCP smoke 181 LOC + 1 fixture sample-document.docx 37KB python-docx reproducible. MIGRATE-01..05 fully shipped. **38/38 plan ship · 30/30 REQ-ID consumed · 7/7 phase complete.** v3.0-a (Phase 1-3) + v3.0-b (Phase 4-7) anti-pivot pattern hoàn tất. Next: `/gsd-complete-milestone v3.0` separate command — archive `.planning/milestones/v3.0-archive/` + reset ROADMAP cho v4.0 backlog (sub-hub split + HA Redis cluster + OCR Vietnamese + streaming /api/ask + comprehensive coverage >80%).*
