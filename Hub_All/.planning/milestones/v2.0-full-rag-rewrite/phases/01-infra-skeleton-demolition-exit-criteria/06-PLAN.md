...
# Plan 06: `.planning/CONVENTIONS.md` cho stack Python M2 (5 section bắt buộc)

## Objective
Tạo `Hub_All/.planning/CONVENTIONS.md` mới cho stack Python M2 với 5 section bắt buộc (theo CORE-05): Test Strategy, Naming Conventions (Python + CocoIndex), APP_NAMESPACE Policy, FastAPI Middleware Order, Logging Fields. Tài liệu này là source of truth chuẩn code cho Phase 2-10. Mỗi section có rule + 1-2 ví dụ DO/DON'T + link Pitfall tương ứng.

## Must-Haves
- File `Hub_All/.planning/CONVENTIONS.md` tồn tại với 5 H2 headings: `## 1. Test Strategy`, `## 2. Naming Conventions (Python + CocoIndex)`, `## 3. APP_NAMESPACE Policy`, `## 4. FastAPI Middleware Order (REVERSED từ Go Gin)`, `## 5. Logging Fields`.
- Mỗi section có rule statement + ví dụ DO/DON'T concrete + reference Pitfall ID (P9/P11/P17/P19).
- Section "Reference Documents" liệt kê 4 file research + link EXIT criteria PROJECT.md.
- Toàn bộ nội dung tiếng Việt có dấu, tên kỹ thuật giữ tiếng Anh.

## Tasks

<task id="01">
<action>
Tạo `Hub_All/.planning/CONVENTIONS.md` với toàn bộ nội dung paste-ready bên dưới. Đây là file MỚI hoàn toàn cho stack Python M2 (file cũ `.planning/codebase/CONVENTIONS.md` cho Go giữ làm tham chiếu khi port — KHÔNG xoá file đó).
...
Khi gặp blocker, đối chiếu với EXIT criteria trong `.planning/PROJECT.md`:
...
- Hub_All/.planning/codebase/CONVENTIONS.md
</read_first>
<acceptance_criteria>
- File `Hub_All/.planning/CONVENTIONS.md` tồn tại.
- `grep -c '^## 1. Test Strategy' Hub_All/.planning/CONVENTIONS.md` ≥ 1.
- `grep -c '^## 2. Naming Conventions' Hub_All/.planning/CONVENTIONS.md` ≥ 1.
- `grep -c '^## 3. APP_NAMESPACE Policy' Hub_All/.planning/CONVENTIONS.md` ≥ 1.
- `grep -c '^## 4. FastAPI Middleware Order' Hub_All/.planning/CONVENTIONS.md` ≥ 1.
- `grep -c '^## 5. Logging Fields' Hub_All/.planning/CONVENTIONS.md` ≥ 1.
- `grep -c 'medinet_prod' Hub_All/.planning/CONVENTIONS.md` ≥ 2 (section 3 + reference).
- `grep -c 'snake_case' Hub_All/.planning/CONVENTIONS.md` ≥ 3 (naming section).
- `grep -c 'medinet_wiki_ingest' Hub_All/.planning/CONVENTIONS.md` ≥ 1 (flow name pin).
- `grep -c 'request_id' Hub_All/.planning/CONVENTIONS.md` ≥ 2 (logging section).
- `grep -c 'P9' Hub_All/.planning/CONVENTIONS.md` ≥ 1 (test strategy pitfall ref).
- `grep -c 'P11' Hub_All/.planning/CONVENTIONS.md` ≥ 1 (middleware pitfall ref).
- `grep -c 'P2' Hub_All/.planning/CONVENTIONS.md` ≥ 1 (cocoindex naming pitfall).
- `grep -c 'P7' Hub_All/.planning/CONVENTIONS.md` ≥ 1 (schema isolation pitfall).
- `grep -c 'P17' Hub_All/.planning/CONVENTIONS.md` ≥ 1 (cosine vs L2 metric pitfall).
- `grep -c 'vector_cosine_ops' Hub_All/.planning/CONVENTIONS.md` ≥ 1 (vector index ops pin).
- `grep -c 'E1' Hub_All/.planning/CONVENTIONS.md` ≥ 1 (EXIT criteria reference).
- `grep -c 'EXIT criteria E1-E5' Hub_All/.planning/CONVENTIONS.md` ≥ 1.
- `grep -c 'M2a EXIT GATE' Hub_All/.planning/CONVENTIONS.md` ≥ 1.
- `grep -c '`pgvector/pgvector:pg16`' Hub_All/.planning/CONVENTIONS.md` ≥ 0 (optional — không bắt buộc trong CONVENTIONS).
- `grep -cE '@cocoindex.flow_def' Hub_All/.planning/CONVENTIONS.md` ≥ 1 (DO example).
- `grep -c 'structlog' Hub_All/.planning/CONVENTIONS.md` ≥ 2 (logging section).
- `wc -l Hub_All/.planning/CONVENTIONS.md` ≥ 200 (medium depth, không quá ngắn).
</acceptance_criteria>
</task>

## Verification
- `test -f Hub_All/.planning/CONVENTIONS.md` exits 0.
- `grep -c '^## ' Hub_All/.planning/CONVENTIONS.md` ≥ 6 (5 section bắt buộc + Reference Documents).
- Tất cả 5 heading bắt buộc xuất hiện: `grep -E '^## [1-5]\.' Hub_All/.planning/CONVENTIONS.md | wc -l` ≥ 5.
- File chứa ít nhất 1 fenced code block Python (DO example): `grep -c '```python' Hub_All/.planning/CONVENTIONS.md` ≥ 3.
- File chứa cảnh báo DO/DON'T: `grep -cE 'DON\047T|DON.T' Hub_All/.planning/CONVENTIONS.md` ≥ 3.
- Reference đến Pitfalls: `grep -c 'PITFALLS.md' Hub_All/.planning/CONVENTIONS.md` ≥ 2.
